...
###############################################################################
# Usage:
# bash $JOD_DIR/scripts/init/launchd/install-jod.sh
#
...
sed -e 's|%JOD_INSTALLATION_NAME%|'"$JOD_INSTALLATION_NAME"'|g' \
  ...
  "$SCRIPT_PATH/com.robypomper.josp.jod.plist.TMPL" >tmp_file
sudo mv tmp_file $PLIST_FILE
sudo chown root $PLIST_FILE
sudo chgrp wheel $PLIST_FILE
